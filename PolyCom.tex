...
\item $\V$ verifies the $\hyperlink{DegreeUpper}{\tt{ZKPoDegUp}}$, the $\hyperlink{DegMono}{\tt{ZKPoDegMono}}$, the \hyperlink{CP}{Chaum-Pedersen} proof and the equation $ a\sta a_{-}\cdot a_{\mr{mid}}\cdot a_{+}.$ \qed \end{enumerate} 
\end{mdframed}
...
\item $\mP$ sends a proof of $\hyperlink{Coeff}{\tt{ZKPoCoef}}[g_1,\;(a_{_{\prod}},\; a_{\deg,2}, \;a_{1,2})]$ 

\noin (to demonstrate that the coefficient of $X^{\deg(f_2)}$ in $f_1(X)\cdot f_2^{\mr{Rev}}(X)$ is the constant committed in $a_{\deg,2}$).


\item $\V$ verifies the $\hyperlink{ZKPoKE}{\tt{ZKPoKE}}$, the $\hyperlink{Prod}{\tt{ZKPoProd}}$, the $\hyperlink{Rev}{\tt{ZKPoRev}}$, the $\hyperlink{DegMono}{\tt{ZKPoDegMono}}$ and the $\hyperlink{Coeff}{\tt{ZKPoCoef}}$. \qed \end{enumerate} 
\end{mdframed}
...
Lastly, the Prover uses the protocol $\hyperlink{Coeff}{\tt{ZKPoCoef}}$ to prove in zero-knowledge that  the coefficient of $X^n$ in $h(X)$ is $0$.
...
\item $\mP$ sends a proof for $\hyperlink{Coeff}{\tt{ZKPoCoef}}[g_1,\;(a_{_{\prod}}, \;a_{\deg},\; \what{a}_{\gamma})]$

\item $\V$ verifies the $\hyperlink{Coeff}{\tt{ZKPoCoef}}$, the $\hyperlink{Prod}{\tt{ZKPoProd}}$, the $\hyperlink{Deg}{\tt{ZKPoDeg}}$s, the $\hyperlink{Const}{\tt{ZKPoConst}}$, the $\hyperlink{ZKOpen}{\tt{ZKPoOpen}}$ and the equations \vs $$ \e(Q\;,\;g_1^{s-\be})\sta \e(a\cdot C_{f(\be)}^{-1}\;,\; g_2)\;\;,\;\; \e(Q_{\gamma}\;,\;g_2^{s-\gamma\cdot \be})\sta \e(a_{\gamma}\cdot C_{f(\be)}^{-1}\;,\; g_2)\;\;,\;\;   \vs $$ and $\e(\what{g}_1\;,\; g_2^{s-1})\sta \e(\wti{g}_1 \cdot g_1^{s}\cdot g_1^{-1}\;,\; g_2)$. \qed
...
\noin - For each index $i\in [\deg(f)]$, the coefficient of $f(X)$ at $X^i$ is non-zero if and only if $i\in \mc{I}$. \vspace{2mm}
...
\noin \textbf{Claim:} The Prover knows a polynomial $f(X)\in \bFp[X]$ and an index set $\mc{I}\sub [\deg(f)]$ such that: 

\noin $\bullet$ $a = g_1^{f(s)}$

\noin $\bullet$ $\wti{a}_{\tt{nz}} = g_1^{\chi_{_{\mc{I}}}(s)}$.

\noin $\bullet$ For every index $i\in [\deg(f)]$, the coefficient of $f(X)$ at $X^i$ is non-zero \textit{if and only if} $i\in \mc{I}$.
...
\noin 2. $\hypertarget{Coeffk}{{\tt{Coef}}_k(f(X)\;,\;i)}(1) = 1 \;\forall\;i$ (this is equivalent to each  row having precisely one $1$). 
\vspace{1.5mm}

\noin 3. $\hypertarget{Coeffkhat}{\what{{\tt{Coef}}}_k(f(X)\;,\;j)}(1) = 1 \;\forall\;j$ (this is equivalent to each column having precisely one $1$). \vspace{2mm}
...
\noin $\bullet$ For each index $i$, the polynomial $\hyperlink{Coeffk}{{\tt{Coef}}_k(f(X)\;,\;i)}$ is valued $1$ at $X=1$.

\noin $\bullet$ For each index $j$, The polynomial $\hyperlink{Coeffkhat}{\what{{\tt{Coef}}}_k(f(X)\;,\;j)}$ is valued $1$ at $X=1$
...
For a committed polynomial $f(X)$, a committed polynomial $w(X)$ and a committed total value $T\in \bFp$, we discuss a protocol that allows a Prover to show that he knows a index set \vs $$\mc{I} = \{n_0,\cdots,n_{\deg(w)} \}\;\sub\; [\deg(f)] \vs $$ such that \vs $$ \sum\limits_{i= 0}^{\deg(w)} {\tt{Coef}}(f(X)\;,\;n_i)\cdot {\tt{Coef}}(w(X)\;,\;i) = T . $$ As with all other protocols in this paper, the proof size and the verification time are both constant. The CRS, however, needs to be of size quadratic in the degree of the polynomial $f(X)$. This downside arises from the CRS needed to commit to the permutation matrix of dimension $(\deg(f)+1)\times (\deg(f)+1)$. 

The Prover computes a permutation $\rho:\;[\deg(f)]\lra [\deg(f)]$ such that $\rho(i) = n_i$ for every $i\in [\deg(w)]$. He sends a \hyperlink{MatCom}{commitment to this permutation} $\rho$ (as a permutation matrix) and uses the protocol $\hyperlink{Perm}{\tt{PoPerm}}$ to show that this is a commitment to some permutation matrix. 
...
\noin 1. $\{{\tt{Coef}}(f(X)\;,\;i)\;:\;i\in\mc{I}\}\sub \mc{D}$. \vspace{1mm} 

\noin 2. $\mc{D} \sub  \{{\tt{Coef}}(f(X)\;,\;i)\;:\;i\in\mc{I}\}$. \vspace{1mm} 

\noin 3. $\mc{D} \cap  \{{\tt{Coef}}(f(X)\;,\;i)\; : \;i\in\mc{I}\} = \emptyset $. \vspace{2mm} 
...
\noin - for every $i\in \mc{I}$, set \vs $${\tt{Coef}}(\wti{S}_{\sigma,\mc{I}}(X)\;,\;i)\;:=\; {\tt{Coef}}({S}_{\sigma,\mc{I}}(X)\;,\;i) \;=\; \sigma(i).$$

\noin - for every $i$ in the complement $\big[ \max(\mc{I})\big]\;\big\backslash\; \mc{I}$, set $m_i:= \max\{j\in \mc{I}: j < i\}$ and set \vs $${\tt{Coef}}(\wti{S}_{\sigma,\mc{I}}(X)\;,\;i)\;:=\; {\tt{Coef}}(S_{\sigma,\mc{I}}(X)\;,\;m_i) \;:=\; \sigma(m_i)  .$$
...
We now describe how the Prover demonstrates the non-decreasing nature of the coefficients of $\wti{S}_{\sigma,\mc{I}}(X)$. The Prover \textit{verifiably} sends a commitment to the product \vs $$(1-X)\cdot \wti{S}_{\sigma,\mc{I}}(X)\; =\;-{\tt{Coef}}(S_{\sigma,\mc{I}}\;,\;0)+ \sum\limits _{i=0}^{\max{(\mc{I})}} \big[{\tt{Coef}}(S_{\sigma,\mc{I}}\;,\;i+1) - {\tt{Coef}}(S_{\sigma,\mc{I}}\;,\;i)\big] \cdot X^{i+1}    .$$ The Prover then uses the protocol for coefficient multisets (\hyperlink{MulSet}{\tt{PoCoefMulSet}}) and the protocol for the radical (\hyperlink{Rad}{ZKPoRad}) 
to show that this committed polynomial $(1-X)\cdot S_{\sigma,\mc{I}}(X) $ is such that its coefficients form a multiset whose underlying set is contained in the interval $[n+1]$.
...
\begin{mdframed}
\begin{Prot} \hypertarget{MulSet}{Protocol for increasing permutation} $(\hypertarget{Perm^}{\tt{PoPoPerm\uparrow}})$\end{Prot}  

\noin \textbf{Parameters:} A pairing ${\bf{e}}:\mb{G}_1\times \mb{G}_2\lra \mb{G}_T$; generators $g_1,\;g_2$ for $\mb{G}_1,\; \mb{G}_2$ respectively. 

\noin The element $A:= g_1^{\prod\limits_{i=0} (s+i)}$ \vspace{1mm}

\noin \textbf{Inputs:} Elements $a, b, C_{\mc{I}} \in \mb{G}_1$

\noin \textbf{Claim:} The Prover knows a polynomial $f(X)\in\bFp[X]$, an index set $\mc{I}$ and a permutation $\sigma$ of $[\big|{\tt{CRS}}\big|]$ such that:

\noin $\bullet$ $a = g_1^{f(s)}$, $b = g_1^{f^{\sigma}(s)}$.

\noin $\bullet$ $C_{\mc{I}} = g_1^{\chi_{_{\mc{I}}}}$.

\noin $\bullet$ The permutation $\sigma$ is increasing on the index set $\mc{I}$.

\begin{enumerate}[wide, labelwidth=!, labelindent=0pt, itemsep=-0.2ex] 

\item The Prover $\mP$ computes a polynomial $S_{\sigma}(X)$ and sends the element $g_1^{S_{\sigma}(s)}$ along with a proof for $\hyperlink{Perm}{\tt{PoPerm}[g_1,\;]}$
...
$$\mc{R}_{\tt{CoefMulSet}}[g_1,\;(a,\;A)] = \left\{\begin{array}{l} \big((a,\;  A\;\in\;\mb{G}_1),\\
...
$$\mc{R}_{\tt{Perm}}[g_1,\;(a,\;b)] = \left\{\begin{array}{l} \big((a,\;  b\;\in\;\mb{G}_1)\;\;,\;\; f(X)\; \in \;\bFp[X]\\
...
%\noin $S_{|{\tt{CRS}}|}$ denotes the group of permutations $[|{\tt{CRS}}|-1]\lra [|{\tt{CRS}}|-1]$. $f^{\sigma}(X)$ denotes the polynomial \vs $$\sum\limits_{i=0}^{\deg(f)} {\tt{Coef}}(f(X)\;,\;\sigma(i))\cdot X^i. $$





\item $\hyperlink{Perm^}{\tt{ZKPoPerm\hspace{-1mm}\uparrow}}$ (\textit{Protocol for increasing permutation}) \vspace{-3mm}
...
$$\mc{R}_{\tt{Subseq}}[g_1,\;(a,\;b),\; (C_{\mc{I}}.\;C_{\mc{J}})] = \left\{\begin{array}{l} \big((a,\;  b,\;C_{\mc{I}},\;C_{\mc{J}}\;\in\;\mb{G}_1),\\

f(X),\; \wti{f}(X)\; \in \;\bFp[X]\;\;,\; \mc{I}, \mc{J}\sub [|{\tt{CRS}}|-1]  \\

\text{Permutation }\sigma:\;[\max(\mc{I}\cup \mc{J})]\;\lra [\max(\mc{I}\cup \mc{J})] \big): \\ 


a = g_1^{f(s)}\;\;,\;\;b = g_1^{\wti{f}(s)} \;\;,\;\;C_{\mc{I}} = g_1^{\chi_{_{\mc{I}}}(s)}\;\;,\;\;C_{\mc{J}} = g_1^{\chi_{_{\mc{J}}}(s)}\\
\forall\; i,j \in \mc{I},\; i <j\;\LRA\; \sigma(i)<\sigma(j) 
